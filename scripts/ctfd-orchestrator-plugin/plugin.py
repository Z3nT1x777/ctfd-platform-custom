...
{% raw %}<script>
(function() {
var MAX_ACTIVE = _CFG.max_active;
...
function ttlPct(sec) { return Math.min(100, Math.round(sec / 36)); }
...
function startTicker() {
  ...
  _ticker = setInterval(function() {
    var minSec = Infinity, minName = '';
    _instances.forEach(function(inst, idx) {
      if (!inst.ttl_remaining_sec) return;
      inst.ttl_remaining_sec = Math.max(0, inst.ttl_remaining_sec - 1);
      var el = document.getElementById('ttl-n-' + idx);
      var bar = document.getElementById('ttl-b-' + idx);
    ...
    if (minSec < Infinity) {
      var el = document.getElementById('stat-min-ttl');
      if (el) el.textContent = fmt(minSec);
    }
  }, 1000);
}
...
async function refreshInstances() {
  try {
    var res = await fetch('/plugins/orchestrator/instances', { cache: 'no-store' });
    ...
    quota.textContent = count + ' / ' + MAX_ACTIVE;
    statR.textContent = count;
    statRSub.textContent = count > 0 ? 'Active' : 'None running';

    var active = _instances.filter(function(i){ return (i.ttl_remaining_sec||0) > 0; });
    if (active.length) {
      var minInst = active.reduce(function(a,b){ return a.ttl_remaining_sec < b.ttl_remaining_sec ? a : b; });
      statT.textContent = fmt(minInst.ttl_remaining_sec);
      statTN.textContent = minInst.challenge_name || '—';
    } else {
      statT.textContent = '—';
      statTN.textContent = count === 0 ? 'No instances' : 'Expired';
    }
    ...
    _instances.forEach(function(inst, idx) {
      ...
      var launchedBy = inst.launched_by_username ? 'by ' + inst.launched_by_username : '';
      var allMeta = metaParts.concat(launchedBy ? [launchedBy] : []).join(' · ');
      ...
      card.innerHTML =
        '<div class="inst-head">' +
          ...
          '<div>' +
            '<div class="inst-name">' + escHtml(inst.challenge_name || '—') + '</div>' +
            '<div class="inst-meta">' + escHtml(allMeta) + '</div>' +
          '</div>' +
        ...
        '</div>' +
        '<div class="inst-body">' +
          '<div class="inst-col"><div class="col-lbl">Connection</div><div class="col-val col-conn">' + escHtml(conn) + '</div></div>' +
          '<div class="inst-col"><div class="col-lbl">TTL remaining</div>' +
            '<div class="ttl-num" id="ttl-n-' + idx + '">' + fmt(sec) + '</div>' +
            '<div class="ttl-bar-wrap"><div class="ttl-bar ' + ttlClass(sec) + '" id="ttl-b-' + idx + '" style="width:' + ttlPct(sec) + '%"></div></div>' +
          '</div>' +
          '<div class="inst-col"><div class="col-lbl">User</div><div class="user-val">' + escHtml(user) + '</div></div>' +
          '<div class="inst-actions">' +
            '<a class="act-btn btn-extend" href="' + extHref + '">⊕ +30m</a>' +
            '<a class="act-btn btn-ssh" href="' + sshHref + '">→ SSH</a>' +
            '<a class="act-btn btn-kill" href="' + killHref + '" onclick="return confirm(\'Kill instance?\')">■ Kill</a>' +
          '</div>' +
        '</div>';
    ...
    document.getElementById('instances-list').innerHTML = '<div class="empty-box">Error loading instances.</div>';
  }
}

async function refreshLeaderboard() {
  try {
    var res = await fetch('/plugins/orchestrator/leaderboard/live', { cache: 'no-store' });
    var data = await res.json();
    ...
    body.innerHTML = rows.map(function(r) {
      return '<tr><td>' + escHtml(r.team_name || r.team_id) + '</td><td>' + r.active_instances +
...
async function refreshQuickLaunch() {
  try {
    var [challRes, instRes] = await Promise.all([
      fetch('/plugins/orchestrator/challenges', { cache: 'no-store' }),
      fetch('/plugins/orchestrator/instances', { cache: 'no-store' }),
    ]);
    var challData = await challRes.json();
    var instData = await instRes.json();
    var runningIds = new Set((instData.instances || []).map(function(i){ return i.challenge_id; }));
    ...
    container.innerHTML = challenges.map(function(ch) {
      var isRun = runningIds.has(ch.id);
      ...
      return '<div class="ql-item' + (isRun ? ' running' : '') +
        ...
        '<div class="ql-name">' + escHtml(ch.name) + '</div>' +
        '<div class="ql-right">' +
