...
**No manual vault setup needed.** `vagrant up` automatically generates `ansible/vars/vault.yml` with random secrets (DB passwords, orchestrator tokens, Grafana password). The generated file is written to your host at `ansible/vars/vault.yml` — read it after provisioning to see the values.

`vagrant up` runs the Ansible playbook automatically:
...
- Syncs OSINT static assets

> **Challenge sync and Docker pre-build are NOT run on `vagrant up`** — they require a CTFd admin token that only exists after you complete the setup wizard (step below). Use the admin panel for both after first boot.
...
3. On your **host machine**, edit `ansible/vars/vault.yml` (auto-generated by `vagrant up`) and set:
   ```yaml
   ctfd_api_token: "your-token-here"
   ```
4. Re-provision to inject the token into the orchestrator (fast — no Docker rebuild):
   ```bash
   vagrant provision
   ```
5. Open the **Orchestrator Admin** panel → http://192.168.56.10/plugins/orchestrator/admin:
   - Click **"Build all images"** — pre-builds all Docker challenge images (~10–20 min, once)
   - Click **"Lancer la sync"** — imports all challenges into CTFd with flags, hints, descriptions

> After this, re-running `vagrant provision` will re-sync challenges automatically (token is stored in vault.yml). You can also trigger sync at any time from the admin panel without re-provisioning.
...
```bash
# Standard re-provision — fast (no Docker rebuild, no pre-build)
vagrant provision

# Or via Ansible directly (even faster, skips VM shell setup)
...
# Force Docker pre-build during provision (slow — normally use admin panel instead):
vagrant ssh -c "sudo bash -c 'cp -a /vagrant/ansible/. /root/ctf-ansible/ && cd /root/ctf-ansible && ansible-playbook -i inventory playbooks/main.yml -e prebuild_on_provision=true'"
```

> **Changing DB passwords after first provision:** MariaDB initialises its data directory on first start and locks in the password. If you change `DB_PASSWORD` or `DB_ROOT_PASSWORD` in vault.yml after the first `vagrant up`, you must wipe the data directory first or MariaDB will fail to authenticate:
> ```bash
> vagrant ssh -c "sudo rm -rf /opt/ctf/ctfd/db_data/"
> vagrant provision
> # CTFd wizard runs again — re-generate admin token and update vault.yml
> ```
> Because `vagrant up` now auto-generates strong random passwords on first boot, this situation only arises if you deliberately replace them.
...
`vagrant up` automatically generates it with **random strong secrets** — no manual step required. The generated file is also written to `ansible/vars/vault.yml` on your host so you can read or edit it.

The playbook warning about default secrets will NOT appear since all values are randomly generated.
